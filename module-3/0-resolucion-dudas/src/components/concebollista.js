...
function App() {
  ...
  const handleresultMessages = (event) => {
    ...
    let macarronesChecked = event.target.form.elements.macarrones.checked;
    let nuecesChecked = event.target.form.elements.nueces.checked;
    let cervezaChecked = event.target.form.elements.cerveza.checked;
    if (cebollaChecked && patatasChecked && huevosChecked) {
      setResultMessage('¡Eres una persona concebollista!');
    } else if (cebollaChecked && patatasChecked && huevosChecked && macarronesChecked && nuecesChecked && cervezaChecked) {
      setResultMessage('Eres un robot sin paladar.');
    } else if (macarronesChecked && nuecesChecked && cervezaChecked){
      setResultMessage('Eres un robot sin paladar.');
    }
  };
  ...
  const handleCheckAll = (event) => {
    ...
    setResultMessage('¿Realmente crees que todos?');
  };
